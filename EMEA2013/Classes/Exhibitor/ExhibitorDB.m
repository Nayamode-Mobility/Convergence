        ...
        if(intEntriesM > 0)
        {
            ...
            for (NSUInteger i = 0; i < intEntriesM; i++)
            {
                ...
                if([Functions ReplaceNUllWithZero:objExhibitor.strExhibitorID]!=0)
                {
                    ...
                    NSString *strSQL = [NSString stringWithFormat:@"Insert Into Exhibitors(ExhibitorID, ExhibitorName, WebsiteURL, LogoURL, CompanyProfile, ExhibitorPhone, BoothNumbers, Address1, Address2, City, State, ZipCode, PhoneNumbers, Fax, Email, FacebookURL, LinkedInURL, TwitterURL, VideoWebCastURL) Values(%i, '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@')",
...
- (NSString*)GetAttendeeExhibitorsJSON
{
    ...
    char *strSQL = "Select AttendeeExhibitors.ExhibitorID, AttendeeExhibitors.IsDeleted From AttendeeExhibitors where AttendeeExhibitors.IsSynced = 0";
